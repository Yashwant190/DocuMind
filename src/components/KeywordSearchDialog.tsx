...
export function KeywordSearchDialog({ 
    ...
    paragraphs.forEach((paragraph, paraIndex) => {
      ...
      keywords.forEach(keyword => {
        ...
        while (true) {
          ...
          const actualStartIndex = documentContent.toLowerCase().indexOf(lowerParagraph) + foundIndex;
  ...
  const performSemanticSearch = async (query: string): Promise<SearchResult[]> => {
    ...
    paragraphs.forEach((paragraph, paraIndex) => {
      const lowerParagraph = paragraph.toLowerCase();
      const words = lowerParagraph.split(/\W+/);
      ...
      semanticTerms.forEach(term => {
        words.forEach((word, wordIndex) => {
          if (word.includes(term) || term.includes(word)) {
            ...
            const wordStart = lowerParagraph.indexOf(word);
            if (wordStart !== -1) {
              matches.push({
                start: wordStart,
      ...
      if (relevanceScore > 0.3 && matches.length > 0) {
        ...
        const actualStartIndex = documentContent.toLowerCase().indexOf(lowerParagraph) + bestMatch.start;
